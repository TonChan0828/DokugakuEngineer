...
function showResult(array $p1, array $p2): array
{
    //プレイヤー１，２のカードを連結
    $player = array_merge($p1, $p2);
    //数値だけを取り出し、ランク付けされた値を取得
    $cardRanks = convertToCardRanks($player);
    //プレイヤーごとにカードを分割
    $playerCardRanks = array_chunk($cardRanks, count($cardRanks) / 2);
    //役を格納 カードは2 or 3枚
...
function isStraight(array $sortCardRanks): bool
{
    ...
    } elseif (count($sortCardRanks) === 2) {
        if ((abs($sortCardRanks[0] - $sortCardRanks[1]) === 1) || (abs($sortCardRanks[0] - $sortCardRanks[1]) === 12)) {
            return true;
        } else {
            return false;
        }
...
function cardRotate(array $sortCardRanks): array
{
    if (count($sortCardRanks) === 3) {
        ...
        if (abs($sortCardRanks[0] - $sortCardRanks[1]) === 12) {
            $tmp = $sortCardRanks[0];
            array_shift(($sortCardRanks));
            $sortCardRanks[] = $tmp;
        }
